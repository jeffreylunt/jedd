 ...
 * it. Measured live 2026-09 (issue #26): two consecutive turns timed out AND
  ...
  const handleBurst = async (batch: IncomingMessage[], waited: BatchWait): Promise<void> => {
    ...
    try {
      ...
      try {
         ...
         * `FAILURE_REPLY_TIMEOUT_MS` above — issue #26 was the apology itself
         * timing out on the normal 15s / 30s budget while the sender was still
         * owed a reply. The 60s here is plumbed through `client.sendText` as an
         * explicit, dedicated value, so nothing in the parent catch chain —
         * including any signal from the model call's controller — can
         * short-circuit it before its time.
         */
