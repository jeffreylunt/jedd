  ...
  const handleBurst = async (batch: IncomingMessage[], waited: BatchWait): Promise<void> => {
    ...
    try {
      ...
      try {
         ...
         * apology ALLOWED to take? Measured twice live 2026-09-10: when the
         ...
         * long as the apology was healthy. The race below puts a hard upper
         * bound on THIS catch — shorter than the connector's internal ceiling,
         * so the deadline is ours and logs as ours — and the rejection below
         * still names whichever side lost the race, so a future regression that
         * re-couples the two timeouts is visible in the log line.
         */
        await Promise.race([
