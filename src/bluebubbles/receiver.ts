...
export class BlueBubblesConnector implements Connector {
   ...
   */
  async sendReporting(toHandle: string, text: string, inReplyTo?: string, timeoutMs?: number): Promise<SendOutcome> {
...
export class ShadowConnector implements Connector {
  ...
  async send(toHandle: string, text: string, _inReplyTo?: string, _record?: SendRecord, _timeoutMs?: number): Promise<void> {
    throw new Error(
