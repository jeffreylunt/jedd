...
// must not be permitted to die too. Measured twice live 2026-09-10.
// ──────────────────────────────────────────────────────────────────────────
...
test('🔴 FAILURE_REPLY_TIMEOUT_MS exists, is short, and is shorter than the connector ceiling', () => {
  ...
  assert.ok(FAILURE_REPLY_TIMEOUT_MS >= 1_000, 'shorter than 1s and a healthy server cannot clear it — the apology dies, just faster');
  /**
   * BlueBubblesClient uses 15_000ms for ordinary sends (`client.ts::call`),
   * with anchored sends sized to 30s. A failure-reply timeout AT or above the
   * ordinary ceiling would not close the live defect — the live defect WAS
   * the connector ceiling firing. The apology timeout must sit BELOW it so
   * its race always has a winner and the log names our deadline, not theirs.
   */
  assert.ok(
...
test('🔴 the shipped catch races the apology send against its own deadline, never awaiting it bare', async () => {
  /**
   * Source scan, mirroring the SIGTERM ones: the wiring of a guarded apology
   * send is what every other test in this file cannot reach. The defect was
   * `await connector.send(...)` with no outer guard — a regression to that
   * line leaves every `failureReply` assertion green, and the only way to
   * catch it is to read the source. This is the test that has to keep it
   * honest.
   *
   * `Promise.race` is required: the alternative ("fire-and-forget with its
   * own deadline") also closes the defect, but it is `void`-ing and would
   * reorder the `finally` below it, which this codebase has stated is not
   * the trade it wants to make. `Promise.race` keeps the `await` and lets
   * the `finally` keep its ordering.
   */
  const main = await readFile(new URL('../src/main.ts', import.meta.url), 'utf8');
  const body = main.slice(main.indexOf('const handleBurst ='), main.indexOf('🔴 EVERY INBOUND MESSAGE GOES THROUGH THE QUEUE'));
  assert.ok(body.length > 0, 'the turn body could not be located — this scan is now measuring nothing');
  ...
  assert.match(
    body,
    /FAILURE_REPLY_TIMEOUT_MS/,
    'the race never resolves into our own constant — somebody hardcoded a timeout, which is also unguarded the day the value changes',
  );
...
test('🔴 a hung connector send cannot hang the catch past FAILURE_REPLY_TIMEOUT_MS', async () => {
   ...
   * exercised is the race shape. A Promise that resolves when the timeout
   * fires is the only practical way to test the "our deadline always fires"
   * half of the contract from outside `main`.
   */
  const startedAt = Date.now();
  const reason = await new Promise<{ ok: false; kind: 'timeout' | 'rejection'; detail: string }>((resolve) => {
    const handle = setTimeout(
      () => resolve({ ok: false, kind: 'timeout', detail: `apology send exceeded the test's deadline` }),
    ...
    Promise.race([
      ...
      new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error(`apology send did not land within ${FAILURE_REPLY_TIMEOUT_MS}ms`)), FAILURE_REPLY_TIMEOUT_MS);
      }),
    ]).then(
      () => resolve({ ok: true, kind: 'timeout', detail: 'unexpectedly resolved' }),
      (e: Error) =>
        resolve({ ok: false, kind: 'rejection', detail: e.message }),
    );
  });
  ...
  assert.equal(
    reason.kind,
    'rejection',
    'the timeout must produce a thrown reason, not just a tick — `main.ts` only learns it happened from the catch',
  );
  assert.match(
    reason.detail,
