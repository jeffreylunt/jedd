...
test('🔴 the catch owns its own abort signal — the apology is not killed by the turn\'s timeout (issue #34)', async () => {
  /**
  ...
  assert.match(
    body,
    /connector\.send\(\s*\n[^]*?\w+\.signal/,
    'connector.send is called without the catch\'s signal — the apology is back on the default 15s, which is what issue #34 is about',
  );
