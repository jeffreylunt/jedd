...
test('🔴 the failure-reply send gets a DEDICATED, longer timeout — issue #26', async () => {
  /**
   * Issue #26: two turns in 24h timed out at the full 900s budget AND THEN the
   * failure-reply send itself timed out at the normal 15s / 30s ceiling, so the
   ...
   * the default budget has a red test in front of it.
   *
  ...
  assert.match(
    ...
    'FAILURE_REPLY_TIMEOUT_MS is missing or no longer 60s — issue #26 was the failure-reply timing out at 15s/30s',
  );
  ...
  // and assert the constant appears between them, on a connector.send call
  // that also names `failureReply(e)`. This covers the layout in both shapes
  // the call site has taken: `connector.send(handle, …, ts)` on one line or
